...
import { Meeting, PresentationA, PresentationAB, PresentationB, Training } from "./Reservation";
import Room from "./Room";
import { TopPageImage } from "./TopPageImage";
export const Incubate = ["/images/incubate_1.webp", "/images/incubate_2.webp", "/images/incubate_3.webp"];
...
const ReservationPage_isAdmin: FC = () => {
    ...
    <>
      <div className="bg-gray-200 h-full">
        <div className="flex flex-col items-center justify-center">
          {/* ------------------ TOP IMAGE ----------------------------- */}
          <TopPageImage title="Would You Prefer ?">
            <div className="mt-10"></div>
            <p className="text-yellow-300 font-medium">Admin page</p>
          </TopPageImage>
          {/* ---------------------------------------------------------- */}
          <div></div>
          ...
          <div className="border-2 my-4 p-4 bg-white w-11/12  shadow-lg rounded-lg">
            ...
            <div className="my-4 flex flex-col lg:flex-row gap-3 justify-center">
              {roomsInfo.slice(0, 6).map((room, index) => (
                <div
                  key={index}
                ...
                >
                  <h2 className="font-bold text-[12px]">{room.name}</h2>
              ...
              <FullCalendar
                ...
                dayCellDidMount={(cellInfo) => {
                  ...
                  const isYearEndNewYear =
                    (date.getMonth() === 11 && date.getDate() >= 29) || (date.getMonth() === 0 && date.getDate() <= 3);
                  if (isSecondSunday || isYearEndNewYear) {
                    cellInfo.el.classList.add("bg-gray-200", "no-hover"); // カレンダーの背景色と同じ色を適用し、マウスオーバーのスタイルを適用しない
                  }
                }}
                datesSet={(dateInfo) => {
                  ...
                  cells.forEach((cell) => {
                    ...
                    if (dateAttr) {
                      ...
                      if (date.getDay() === 0 && Math.floor((date.getDate() - 1) / 7) === 1) {
                        cell.classList.add("bg-gray-200", "no-hover"); // カレンダーの背景色と同じ色を適用し、マウスオーバーのスタイルを適用しない
                      }
                      if (
                      ...
                      <div className="px-8 pt-6 pb-8 mb-4 flex flex-col">
                        ...
                        <div className="flex items-center justify-center gap-4">
                          <form
                            ...
                            method="POST"
                            className="gid text-center bg-yellow-300 hover:bg-yellow-400 text-black font-bold py-1 px-4 w-1/2 rounded-lg shadow"
                          >
...
export default ReservationPage_isAdmin;
